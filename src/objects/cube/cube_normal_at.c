...
/*   Updated: 2023/08/17 21:13:38 by arobu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_tuple cube_normal_at(t_shape *shape, t_tuple isec_point)
{
	...
	if (is_approx_equal(max, abs_x, M_EPSILON))
		normal = tuple_new_vector(local_point.x, 0, 0);
	else if (is_approx_equal(max, abs_y, M_EPSILON))
		normal = tuple_new_vector(0, local_point.y, 0);
	else if (is_approx_equal(max, abs_z, M_EPSILON))
		normal = tuple_new_vector(0, 0, local_point.z);
	world_normal = tuple_normalize(matrix_multiply_tuple(shape->transform.tf, normal));
	world_normal.w = 0;
	return (world_normal);
}